...
class ModelOutput:
    ...
    def eval_age_contact_matrix(
        ...
        # Personen pro Altersgruppe zählen
        ...
        for location in locations:
            ...
            if len(all_df_average_edges) > 0:
                ...
                r = compare_contact_matrices(m_sim_5, m_prem, step_size=step_size, only_correlation=only_correlation, same_color_scale=same_color_scale)
                if only_correlation:    
    ...
    def eval_incidence(self, sim_date_col="date_symptomatic", plot=True, per_replication=False) -> dict:
        ...
        def prepare_df_sim_incidence(df_agents):
            ...
            df_sim_incidence = df_sim_incidence.reset_index().groupby(pd.Grouper(key="date", freq="W-MON", closed='left')).mean().reset_index()
        ...
        def prepare_df_rki_incidence():
            ...
            df = df.groupby(pd.Grouper(key="Meldedatum", freq="W-MON", closed='left')).mean().reset_index()
            df["Meldedatum"] = pd.to_datetime(df["Meldedatum"]).apply(lambda x: x.date())
            df = df.loc[(df["Meldedatum"] >= dt.date(2022, 6, 1)) & (df["Meldedatum"] < dt.date(2022, 8, 1)),:].reset_index()
