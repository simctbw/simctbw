...
class Agent:
    ...
    def clean_temp_diary(self, n_days=1, environment=None):
        ...
        # Delete only those entries which are too far in the past
            ...
            self.temp_contact_diary = [
                entry for entry in self.temp_contact_diary 
    ...
    def get_df_daily_contacts(self, location: Optional[str] = None) -> Optional[pd.DataFrame]:
        ...
        for location_category in self.location_dict:
            if location_category != "home":
                ...
                if (cond is None or cond is True or cond(environment, self)):
                    ...
                    location = random.choice(self.location_dict[location_category]["objects"])
...
class PandemicAgent(Agent):
    ...
    def calculate_transition_dates(self) -> None:
        """
        ...
        DURATION_INFECTIOUS = 2 + random.randint(-1,1)
    ...
    def update_infection(
        ...
        if self.contact_traced:
            ...
            if (environment.current_date - self.contact_tracing_date).days > n_days_contact_tracing_isolation:
                self.contact_traced = False
                self.contact_tracing_date = None
        ...
        if self.infection_status == INFECTED:
            ...
            if self.date_infectious is None:
                ...
                self.calculate_transition_dates()
        ...
        # if the agent is currently presymptomatic infectious and it is about time to change to the next status (ill)
        elif self.infection_status == INFECTIOUS and environment.current_date >= self.date_ill:
            ...
            if environment.current_date >= self.date_ill:
                ...
                if self.contact_tracer:
                    trace = False
                    if contact_tracing_detected_infections in ["ill", "ill_symptomatic"] and self.infection_status == ILL_SYMPTOMATIC:
                        trace = True
                    elif contact_tracing_detected_infections == "ill" and self.infection_status == ILL_ASYMPTOMATIC:
                        trace = True

                    if trace and contact_tracing_allowed:
                        self.trace_contacts(
        ...
        elif self.infection_status == ILL_SYMPTOMATIC or self.infection_status == ILL_ASYMPTOMATIC:
        ...
        elif self.infection_status == ICU and environment.current_date >= self.date_recovered:
            self.infection_status = RECOVERED
            environment.n_currently_infected -= 1
    ...
    def get_infected(
        ...
        self.date_infection = (environment.current_date if date_infection is None else date_infection)
        ...
        self.update_infection(environment)
        ...
        if self.infection_status in INFECTIOUS_STATES:
            ...
            for entry in self.temp_contact_diary:
                ...
                if agent_j.infection_status == SUSCEPTIBLE:
                    ...
                    infection_probability = infection_probability_dict[entry["infection_probability_category"]] if infection_probability is None else infection_probability
                    ...
                    if self.contact_tracer and self.infection_status == ILL_SYMPTOMATIC or self.contact_traced:
                        p = p * 0.01
                    ...
                    if random.random() < p:
                        ...
                        agent_j.get_infected(environment=environment, infection_location=entry["location"], infection_source=self)
    ...
    def trace_contacts(
        ...
        # only if the agent is not himself currently under quarantine due to contact tracing and if the current date is at least the official start date (and not within the warm up phase)
        if not self.contact_traced and environment.current_date >= self.model.start_date:
            
            # go through the contact tracing diary and keep only those contacts that were made today, yesterday or the day before yesterday
            self.contact_tracing_diary = [entry for entry in self.contact_tracing_diary if (environment.current_date - entry["date"]).days <= 2]

            # sort contacts in contact tracing diary by the contact intensity
            self.contact_tracing_diary.sort(key=lambda entry: entry["weight"], reverse=True)
            ...
            for entry in temp_contact_tracing_diary:
                if entry["id_j"] not in collision:
                    ...
                    collision.append(entry["id_j"])

            # if there are any contacts to trace
            if len(self.contact_tracing_diary) > 0:
                
                # increase contact tracing counter
                self.n_times_contact_tracing += 1

                # for each contact to trace, get the agent-object and add attribute-information to the diary entry (to sort the entries in the next step)
                ...
                self.contact_tracing_diary.sort(key=lambda entry: entry[contact_tracing_contact_selection_mode], reverse=True)

                # calculate the number of contacts which should be traced
                n_agents_traced = round(len(self.contact_tracing_diary) * contact_tracing_contact_selection_frac)
                
                # for each agent to be traced
                for i in range(n_agents_traced):

                    # get the agent
                    agent_j = environment.agents_dict[self.contact_tracing_diary[i]["id_j"]]
                    ...
                    if not agent_j.contact_traced:
                        ...
                        agent_j.contact_tracing_dates_list.append(environment.current_date)
                        self.n_contacts_traced += 1

                        environment.contact_tracing_currently_traced += 1
