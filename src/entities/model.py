...
class Model:
    def __init__(
        ...
        self.location_declarants: Optional[List[LocationDeclarant]] = location_declarants
        ...
        self.population_seed: int = (population_seed if population_seed is not None else random.randint(0, 1000000))
        ...
        self.contact_tracing_contact_selection_mode = contact_tracing_contact_selection_mode
        self.contact_tracing_contact_selection_frac = contact_tracing_contact_selection_frac
        self.contact_tracing_detected_infections = contact_tracing_detected_infections
        self.contact_tracing_max_currently_traced: int = contact_tracing_max_currently_traced
        ...
        # no?
        if not self.replace_population:
            # create population now
            self.agents_and_locations = self.population_maker.create_agents_and_locations(n_agents=self.n_agents)
        # yes?
        else:
            # do not create population now, but in each replication
            self.agents_and_locations = None
        
        # check if this is an infection simulation
        if isinstance(initial_infections, int) and initial_infections > 0 or isinstance(initial_infections, dict):
            assert infection_probability is not None or infection_probability_dict is not None, "No infection probability (dict) specified."
    ...
    def add_pred_value(self, agents: List[Agent]) -> List[Agent]:
        ...
        for agent in agents:
            ...
            agent.pred_value = utils.predict_caused_infections(age_rki=agent.age_group_rki, household_size=agent.household_size)
            agent.age_group_rki_inc_rank_emp = utils.recode_age_group_rki_to_inc_rank_emp(agent.age_group_rki)
            agent.age_group_rki_inc_rank_sim = utils.recode_age_group_rki_to_inc_rank_sim(agent.age_group_rki)
            agent.age_group_rki_age_rank = utils.recode_age_group_rki_to_age_rank(agent.age_group_rki)
        return agents
    

    def sort_agents(self, agents: List[Agent], sort_mode: str, add_random: bool = False) -> List[Agent]:
        ...
        def add_jitter(execute, agents, agent_attr, low=0.000001, high=0.00001):
            if execute:
                for agent in agents:
                    setattr(agent, agent_attr, getattr(agent, agent_attr) + random.uniform(low, high))
        ...
        # large households first
            ...
            add_jitter(execute=add_random, agents=agents, agent_attr="age_group_rki_age_rank")
            agents.sort(key=lambda x: x.age_group_rki_age_rank, reverse=False)
        
        elif sort_mode == "inc_rank_emp":
            add_jitter(execute=add_random, agents=agents, agent_attr="age_group_rki_inc_rank_emp")
            agents.sort(key=lambda x: x.age_group_rki_inc_rank_emp, reverse=False)
        
        elif sort_mode == "inc_rank_sim":
            add_jitter(execute=add_random, agents=agents, agent_attr="age_group_rki_inc_rank_sim")
    ...
    def create_model_output(self, list_of_replication_output_dicts: List[dict]) -> ModelOutput:
    ...
    def save_on_disk(self, model_output: ModelOutput, rep_index=None) -> None:
        ...
        if self.mode != "calib":
            ...
            # add some additional information to the df_agents
            df_agents["master_model_index"] = self.master_model_index
            df_agents["unique_agent_id"] = df_agents["master_model_index"].astype(str) + "_" + df_agents["id"].astype(str)
            df_agents["unique_household_id"] = df_agents["master_model_index"].astype(str) + "_" + df_agents["household_id"].astype(str)
            df_agents["name_of_run"] = self.name_of_run
            df_agents["lockdown_population_sort_mode"] = str(self.lockdown_population_sort_mode)
            df_agents["lockdown_frac"] = self.lockdown_frac
            df_agents["vaccination_population_sort_mode"] = str(self.vaccination_population_sort_mode)
            df_agents["vaccination_frac"] = self.vaccination_frac
            df_agents["vaccination_efficacy"] = self.vaccination_efficacy
            df_agents["population_selection_mode"] = self.population_selection_mode
            df_agents["contact_tracing_population_sort_mode"] = str(self.contact_tracing_population_sort_mode)
            df_agents["contact_tracing_frac"] = self.contact_tracing_frac
            df_agents["contact_tracing_contact_selection_mode"] = str(self.contact_tracing_contact_selection_mode)
            df_agents["contact_tracing_contact_selection_frac"] = self.contact_tracing_contact_selection_frac
            df_agents["contact_tracing_detected_infections"] = self.contact_tracing_detected_infections
            df_agents["n_days_contact_tracing_isolation"] = self.n_days_contact_tracing_isolation
            ...
            file_name = f"df_agents_{str(self.master_model_index)}_{str(rep_index)}.csv"
            df_agents.to_csv(Path.joinpath(self.folder_path, file_name), index=False)
        ...
        file_name = f"df_params_{str(self.master_model_index)}_{str(rep_index)}.csv"
        df_params.to_csv(Path.joinpath(self.folder_path, file_name), index=False)
    ...
    def run(self) -> ModelOutput:
        ...
        for i in range(self.n_replications):
            ...
            if self.output_type == "save_rep_on_disk":
                model_output = self.create_model_output(list_of_replication_output_dicts)
    ...
    def replication(self, internal_replication_index: str) -> dict:
        ...
        # create the population and locations
        if self.agents_and_locations is None:
            agents_and_locations = self.population_maker.create_agents_and_locations(n_agents=self.n_agents)
        else:
            agents_and_locations = copy.deepcopy(self.agents_and_locations)
        ...
        if self.lockdown_population_sort_mode is not None:
            agents = self.sort_agents(agents=agents, sort_mode=self.lockdown_population_sort_mode)
            agents = self.select_agents(agents=agents, frac=self.lockdown_frac, attribute="lockdown", value=True, how=self.population_selection_mode) 
        if self.vaccination_population_sort_mode is not None:
            agents = self.sort_agents(agents=agents, sort_mode=self.vaccination_population_sort_mode)
            agents = self.select_agents(agents=agents, frac=self.vaccination_frac, attribute="vaccinated", value=True, how=self.population_selection_mode)
        if self.contact_tracing_population_sort_mode is not None:
            agents = self.sort_agents(agents=agents, sort_mode=self.contact_tracing_population_sort_mode)
            agents = self.select_agents(agents=agents, frac=self.contact_tracing_frac, attribute="contact_tracer", value=True, how=self.population_selection_mode)
        ...
        environment = Environment(
            current_date=self.start_date - dt.timedelta(days=self.n_days_warm_up_period),
            agents=agents,
            timetable=self.timetable,
            )
        environment.update_current_interventions()
        
        # Calculate the number of days that will be simulated
        n_simulated_days = (self.end_date - self.start_date).days + 1 + self.n_days_warm_up_period
        agents_and_locations.n_simulated_days = n_simulated_days

        # initial infections
        if self.initial_infections is not None:
            # simple random selection
            if (isinstance(self.initial_infections, float) or isinstance(self.initial_infections, int)) and self.initial_infections > 0:
                patients_zero = random.sample(agents, round(self.initial_infections*self.n_agents))
            ...
            # group specific random selection
                ...
                for key in self.initial_infections.keys():
                    if key != "attribute":
                        subpopulation = [agent for agent in agents if getattr(agent, group_attribute) == key]
            ...
            for agent in patients_zero:
                ...
                date_infection = random.choice(utils.dates_between(environment.current_date-dt.timedelta(days=8), environment.current_date-dt.timedelta(days=1)))
                agent.get_infected(environment=environment, infection_source=None, date_infection=date_infection)
                agent.patient0 = True
        ...
        for day in range(n_simulated_days):
            ...
            if not self.stop_sim_if_0_infections or (self.stop_sim_if_0_infections and environment.n_currently_infected > 0):

                # each agent: visit locations that are on the schedule for this day
                for agent in agents:
                    agent.visit(environment=environment)
                
                save_contact_network = (True if day < self.n_days_save_contact_network else False)
                ...
                if self.initial_infections is not None:
                    ...
                    for agent in agents:
                        agent.infect(
                            ...
                            contact_tracing = (True if self.contact_tracing_population_sort_mode is not None else False),
                        ...
                        agent.update_infection(
                            ...
                            contact_tracing_allowed=(
                                True 
                                if self.contact_tracing_max_currently_traced is None or environment.contact_tracing_currently_traced < self.contact_tracing_max_currently_traced
                                else False
                                ),
                ...
                environment.contact_tracing_currently_traced = 0
                ...
                list_n_currently_infected.append(environment.n_currently_infected)
        ...
        for agent in agents:
            ...
            # calculate mean and max "hours not at home"
            agent.mean_hours_not_at_home = sum(agent.list_hours_not_at_home) / len(agent.list_hours_not_at_home)
            agent.max_hours_not_at_home = max(agent.list_hours_not_at_home)
