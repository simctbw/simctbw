...
class PopMaker:
    ...
    def create_agents(self, n_agents: int, agent_class: Agent = Agent) -> List[Agent]:
        ...
        if self.household_weight_column is not None:
            ...
            df_hids_and_weights = df.drop_duplicates(subset=[self.household_id_column]).loc[
                :, [self.household_id_column, self.household_weight_column]
            ]
            ...
            for i, location_declarant in enumerate(self.location_declarants):
                ...
                if location_declarant.association_condition is not None:
                    category_related_agents = [
                        agent for agent in agents if location_declarant.association_condition(agent)
                    ]
                else:
                ...
                for location_type in unique_location_types:
                    ...
                    type_related_agents = [
                        agent
                        for agent in category_related_agents
                        if agent.location_dict[location_declarant.category]["type"] == location_type
                    ]
                    ...
                    if location_declarant.n_agents_per_location is not None:
                        n_locations_of_this_type = max(
                            len(type_related_agents) // location_declarant.n_agents_per_location,
                            1,
                        )
                    else:
                    ...
                    if location_declarant.existing_location_object is not None:
                        ...
                        for agent in type_related_agents:
                            existing_location_object = location_declarant.existing_location_object(
                                locations,
                            )
                            agent.location_dict[location_declarant.category]["objects"] = [
