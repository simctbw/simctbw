...
def model_func(
    ...
    n_days_save_contact_network=14,
    inf_p_shift=0,
    scaling_factor=1,
    ...
    if mode == "eval":
        ...
        params["infection_probability"] = trial.suggest_float("infection_probability", 0.32, 0.34)
    ...
    model = Model(
        ...
        end_date=end_date,
        df_agents_in_households=def_population.get_soep4sim(year=2018, area="BW"),
    ...
    if mode == "calib":
        df_params = pd.concat(
            [pd.read_csv(Path.joinpath(run_folder_path, f'df_params_{str(master_model_index)}_{str(rep_index)}.csv')) for rep_index in range(model.n_replications)]
            )
        ...
        for rep_index in range(model.n_replications):
            os.remove(
                Path.joinpath(run_folder_path, f'df_params_{str(master_model_index)}_{str(rep_index)}.csv')
                )
